...
public class KafkaUtils {
  ...
  private static Properties loadExternalProps(String filename) {
    try (Reader propertiesReader = FileUtils.getReader(filename, StandardCharsets.UTF_8.name())) {
      ...
      throw new IllegalStateException(
          String.format("Cannot load kafka property file %s.", filename));
    }
  }

  // access set to package so unit tests can validate created properties without initializing a
  // Consumer
  static Properties createConsumerProps(Config config, String clientId) {
    ...
    Properties consumerProps = new Properties();
    consumerProps.put(
        ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, config.getString("kafka.bootstrapServers"));
    if (config.hasPath("kafka.securityProtocol")) {
      consumerProps.put(
          CommonClientConfigs.SECURITY_PROTOCOL_CONFIG, config.getString("kafka.securityProtocol"));
    }
  ...
  static Properties createProducerProps(Config config) {
    ...
    producerProps.put(
        ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, config.getString("kafka.bootstrapServers"));
    if (config.hasPath("kafka.securityProtocol")) {
      producerProps.put(
          CommonClientConfigs.SECURITY_PROTOCOL_CONFIG, config.getString("kafka.securityProtocol"));
    }
    producerProps.put(
        ProducerConfig.MAX_REQUEST_SIZE_CONFIG, config.getInt("kafka.maxRequestSize"));
  ...
  public static KafkaProducer<String, Document> createDocumentProducer(Config config) {
    ...
    String serializerClass =
        config.hasPath("kafka.documentSerializer")
  ...
  public static void validateAtMostOneRecord(ConsumerRecords<?, ?> consumerRecords)
      throws Exception {
  ...
  public static void createEventTopic(Config config, String pipelineName, String runId)
    ...
    try (Admin kafkaAdminClient = Admin.create(props)) {
      ...
      CreateTopicsResult result =
          kafkaAdminClient.createTopics(List.of(eventTopic), new CreateTopicsOptions());
