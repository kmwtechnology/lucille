 ...
 * destination, and 3) send Events relating to Documents being processed (e.g. if a child Document
 * is created, if a Document can't be processed, etc.)
 */
public interface WorkerMessageManager {
   ...
   * but apply a timeout of several milliseconds to several seconds so that this method can be
   * called from within a polling loop that periodically checks other conditions even when no
  ...
  /**
   * Indicates that Documents previously retrieved by pollDocToProcess() have been processed and
   * should not be returned by subsequent calls.
   */
