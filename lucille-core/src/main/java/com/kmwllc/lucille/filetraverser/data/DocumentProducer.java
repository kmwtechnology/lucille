...
/**
 * An interface for classes that handle the production of {@link Document}s by traversing a file tree. These {@code Documents} can
 * be sent to Kafka using a {@link org.apache.kafka.clients.producer.Producer} set up with a {@link DocumentSerializer}, and
 * consumed using a {@link org.apache.kafka.clients.consumer.Consumer} set up with a {@link DocumentDeserializer}.
 *
 * <p>Several methods are defined as {@code default} for convenience and may be overridden in the
 * implementing class, but the only required method is the {@link this#produceDocuments(Path, Document)} method, which handles the
 * creation of the documents from the given file.
 */
public interface DocumentProducer {
  ...
  /**
   * Produces a list of documents from the given file. The {@code doc} param should be set up with basic file information, so if
   * multiple documents are going to be produced, the {@code doc} should be cloned.
   *
   ...
   * and add a tombstone in the list of documents returned. If a {@link DocumentException} or {@link IOException} occur, the
   * FileTraverser attempts to send a basic tombstone for the document with basic information. Therefore, if this error occurs while
   * processing a child document, throwing an error will prevent the processing of all remaining child documents, and data that may
   * have been usable will be skipped.
   *
   * @param file The file currently being processed
   * @param doc  The document to-be-sent/cloned that should be set up with some basic information from the caller
  ...
  /**
   * Returns true if the file's size should be checked before running document producer on file, otherwise, no check will be
   * performed and doc producer will always be run.
   *
   ...
   *
   * @return true if the file's size should be checked before running the document producer, false otherwise
   */
  ...
  /**
   * Creates a tombstone of the provided document if an error occurs, logging the error in the "errors" field and ensuring the
   * {@link Document#ID_FIELD} and {@link FileTraverser#FILE_PATH} fields are set.
   *
  ...
  /**
   * Creates a new Document producer based off of the provided {@code type} after uppercasing it. If an unknown type is provided, a
   * warning is logged and a {@link DefaultDocumentProducer} is returned instead.
   *
