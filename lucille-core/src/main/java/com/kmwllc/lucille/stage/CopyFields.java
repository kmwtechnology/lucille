...
/**
 * Copies values from a given set of source fields to a given set of destination fields. If the same number of fields are supplied
 * for both sources and destinations, the fields will be copied from source_1 to dest_1 and source_2 to dest_2. If either source or
 * dest has only one field value, and the other has several, all of the fields will be copied to/from the same field.
 *
 * <p>Config Parameters:
 *
 ...
 * supply one destination field for all of the source fields to be mapped into. - update_mode (String, Optional) : Determines how
 * writing will be handling if the destination field is already populated. Can be 'overwrite', 'append' or 'skip'. Defaults to
 * 'overwrite'.
 */
