...
/**
 * Executes a Lucille run. A run is a sequential execution of one or more Connectors. During a run,
 * all the work generated by one Connector must be complete before the next Connector begins. A run
 * should stop if any Connector fails. A Connector is considered to have failed if any of its
 * lifecycle methods throw an Exception. Importantly, a connector is NOT considered to have failed
 * if one or more of the documents it publishes encountere an error during pipeline exeuction or
 * indexing.
 * <p>
 * In a distributed deployment, there will be one or more Worker processes and one or more Indexer
 * processes that stay alive indefinitely and constantly poll for work. A Runner can then execute a
 * sequence of connectors which pump work into the system. The work from this particular "run"
 * should then be handled by the existing Worker and Indexer. The Runner will terminate when all the
 * work it generated was complete, while the Worker and Indexer would stay alive.
 * <p>
 * For simpler deployments, Runner provides a way to execute an end-to-end run without requiring
 * that the Worker and Indexer have been started as separate processes. Instead, the Worker(s) and
 * Indexer are launched as threads in the Runner's own JVM. This can be done in a fully local mode,
 * where all message traffic is stored in in-memory queues, or in Kafka mode, where message traffic
 * flows through an external deployment of Kafka.
 * <p>
 * A local end-to-end run involves a total of 4 threads for each connector:
 * <p>
 * 1) a Worker thread polls for documents to process and runs them through the pipeline 2) an
 * Indexer thread polls for processed documents and indexes them 3) a Connector thread reads data
 * from a source, generates documents, and publishes them; Connectors are run sequentially so there
 * will be at most one Connector thread at any time 4) the main thread launches the other threads,
 * and then uses the Publisher to poll for Events and wait for completion of the run
 */
public class Runner {
   ...
   * <p>
   * no args: pipelines workers and indexers will be executed in separate threads within the same
   * JVM; communication between components will take place in memory and Kafka will not be used
   * <p>
   * -usekafka: connectors will be run, sending documents and receiving events via Kafka. Pipeline
   * workers and indexers will not be run. The assumption is that these have been deployed as
   * separate processes.
   * <p>
   * -local: modifies -usekafka so that workers and indexers are started as separate threads within
   * the same JVM; kafka is still used for communication between them.
   * <p>
   * -render: prints out the effective/actual config in the exact form it will be seen by Lucille
   * during the run
   */
  public static void main(String[] args) throws Exception {
    Options cliOptions = new Options()
        .addOption(Option.builder("usekafka").hasArg(false)
            .desc("Use Kafka for inter-component communication and don't execute pipelines locally")
            .build())
    ...
    try {
      ...
      try (StringWriter writer = new StringWriter();
        ...
        new HelpFormatter().printHelp(printer, 256, "Runner", header, cliOptions, 2, 10, "", true);
   ...
   * <p>
   * Run in a local mode where message traffic is kept in memory and there is no communication with
   * external systems like Kafka. Sending to solr is bypassed.
   * <p>
   * Return a TestMessenger for each connector that can be used to review the messages that were
   * sent between various components during that connector's execution.
   */
  ...
  public static String stringifyValidation(Map<String, List<Exception>> exceptions) {
    if (exceptions.entrySet().stream().allMatch(e -> e.getValue().isEmpty())) {
      ...
      StringBuilder message = new StringBuilder(
          "Configuration is invalid. Printing the list of exceptions for each pipeline\n");

      // TODO: Possibly add list of connectors which are being used, their piplines, and if those
      // pipelines are valid
      for (Map.Entry<String, List<Exception>> entry : exceptions.entrySet()) {
        message.append("\tPipeline: ").append(entry.getKey()).append("\tError count: ")
            .append(entry.getValue().size()).append("\n");
        int i = 1;

        for (Exception e : entry.getValue()) {
          message.append("\t\tException ").append(i++).append(": ").append(e.getMessage())
              .append("\n");
        }
      }
  ...
  public static Map<String, List<Exception>> runInValidationMode(String configName)
      throws Exception {
   ...
   * TODO: create a validateEverything(Config config) method which makes a call to this method as
   * well as calls which validate other parts of the config
   */
  ...
  public static RunResult run(Config config, RunType type, String runId) throws Exception {
    ...
    for (Connector connector : connectors) {
      ...
      if (RunType.TEST.equals(type)) {
        ...
        workerMessengerFactory =
            WorkerMessengerFactory.getKafkaFactory(config, connector.getPipelineName());
        indexerMessengerFactory =
            IndexerMessengerFactory.getKafkaFactory(config, connector.getPipelineName());
        publisherMessengerFactory = PublisherMessengerFactory.getKafkaFactory(config);
      }

      ConnectorResult result = runConnectorWithComponents(config, runId, connector,
          workerMessengerFactory, indexerMessengerFactory, publisherMessengerFactory,
          startWorkerAndIndexer, bypassSolr);
  ...
  /**
   * Runs the designated Connector. Returns a successful ConnectorResult only when 1) the Connector
   * has finished generating documents, and 2) all of the documents (and any generated children)
   * have reached an end state in the workflow: either being indexed or erroring-out. Returns a
   * failing ConnectorResult if any connector lifecycle method throws an exception or the publisher
   * itself fails
   */
  public static ConnectorResult runConnector(Config config, String runId, Connector connector,
      Publisher publisher) {
    try {
      ...
      try {
        ...
        if (publisher != null) {
          try {
            ...
            return new ConnectorResult(connector, publisher, false,
                "Error closing connector and publisher");
          }
        }
  ...
  /**
   * Helper used by runConnector(). Performs the following steps: 1) call connector.preExecute() 2)
   * launch a thread that calls connector.execute() with the given publisher, and then
   * publisher.flush() 3) call publisher.waitForCompletion() 4) call connector.postExecute()
   */
  private static ConnectorResult runConnectorInternal(Config config, String runId,
      Connector connector, Publisher publisher) {
    ...
    // the publisher could be null if we are running a connector that has no associated pipeline and
    // therefore
    ...
    if (publisher == null) {
      ...
      try {
        ConnectorThread connectorThread =
            new ConnectorThread(connector, publisher, ThreadNameUtils.createName("Connector"));
        connectorThread.start();
        final int connectorTimeout =
            config.hasPath("runner.connectorTimeout") ? config.getInt("runner.connectorTimeout")
  ...
  /**
   * Wrapper around runConnector() that starts and stops the other components necessary for a
   * complete connector execution; specifically, a WorkerPool and an Indexer.
   */
  private static ConnectorResult runConnectorWithComponents(Config config, String runId,
      Connector connector, WorkerMessengerFactory workerMessengerFactory,
      IndexerMessengerFactory indexerMessengerFactory,
      PublisherMessengerFactory publisherMessengerFactory, boolean startWorkerAndIndexer,
    ...
    try {
      ...
      // create a common metrics naming prefix to be used by all components that will be collecting
      // metrics,
      ...
      if (startWorkerAndIndexer && connector.getPipelineName() != null) {
        ...
        try {
          ...
          return new ConnectorResult(connector, publisher, false,
              "Error starting workers for pipeline " + pipelineName);
        }

        try {
          IndexerMessenger indexerMessenger = indexerMessengerFactory.create();
          indexer =
              IndexerFactory.fromConfig(config, indexerMessenger, bypassIndexer, metricsPrefix);
        } catch (Exception e) {
          log.error("Error creating indexer from config.", e);
          return new ConnectorResult(connector, publisher, false,
              "Error creating indexer from config.");
        }
