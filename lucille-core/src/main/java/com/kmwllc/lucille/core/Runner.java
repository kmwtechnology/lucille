 ...
 * <p>
 * In a distributed deployment, there will be one or more Worker processes and one or more Indexer processes that stay alive
 * indefinitely and constantly poll for work. A Runner can then execute a sequence of connectors which pump work into the system.
 * The work from this particular "run" should then be handled by the existing Worker and Indexer. The Runner will terminate when all
 * the work it generated was complete, while the Worker and Indexer would stay alive.
 * <p>
 * For simpler deployments, Runner provides a way to execute an end-to-end run without requiring that the Worker and Indexer have
 * been started as separate processes. Instead, the Worker(s) and Indexer are launched as threads in the Runner's own JVM. This can
 * be done in a fully local mode, where all message traffic is stored in in-memory queues, or in Kafka mode, where message traffic
 * flows through an external deployment of Kafka.
 * <p>
 * A local end-to-end run involves a total of 4 threads for each connector:
 * <p>
 * 1) a Worker thread polls for documents to process and runs them through the pipeline 2) an Indexer thread polls for processed
 * documents and indexes them 3) a Connector thread reads data from a source, generates documents, and publishes them; Connectors
 * are run sequentially so there will be at most one Connector thread at any time 4) the main thread launches the other threads, and
 * then uses the Publisher to poll for Events and wait for completion of the run
 */
public class Runner {
  ...
  public enum RunType {
    ...
    KAFKA_DISTRIBUTED // assume Workers/Indexers were started separately (don't launch threads); have all components communicate via
                      // Kafka
  }
   ...
   * <p>
   * no args: pipelines workers and indexers will be executed in separate threads within the same JVM; communication between
   * components will take place in memory and Kafka will not be used
   * <p>
   * -usekafka: connectors will be run, sending documents and receiving events via Kafka. Pipeline workers and indexers will not be
   * run. The assumption is that these have been deployed as separate processes.
   * <p>
   * -local: modifies -usekafka so that workers and indexers are started as separate threads within the same JVM; kafka is still
  ...
  public static void main(String[] args) throws Exception {
    Options cliOptions = new Options()
        ...
        .addOption(Option.builder("local").hasArg(false).desc("Modifies usekafka mode to execute pipelines locally").build())
        .addOption(Option.builder("validate").hasArg(false).desc("Validate the configuration and exit").build());
    ...
    try {
      ...
      Slf4jReporter.forRegistry(SharedMetricRegistries.getOrCreate(LogUtils.METRICS_REG)).outputTo(log)
          .withLoggingLevel(getMetricsLoggingLevel(config)).build().report();
   ...
   * <p>
   * Run in a local mode where message traffic is kept in memory and there is no communication with external systems like Kafka.
   * Sending to solr is bypassed.
   * <p>
   * Return a TestMessenger for each connector that can be used to review the messages that were sent between various components
   * during that connector's execution.
   */
  ...
  public static Map<String, List<Exception>> validatePipelines(Config config) throws Exception {
    ...
    for (Connector connector : Connector.fromConfig(config)) {
      log.info("Validating stages for connector \"" + connector.getName() + "\" pipeline\" " + connector.getPipelineName() + "\"");
  ...
  public static RunResult run(Config config, RunType type) throws Exception {
    ...
    for (Connector connector : connectors) {
      ...
      ConnectorResult result = runConnectorWithComponents(config, runId, connector, workerMessengerFactory, indexerMessengerFactory,
          publisherMessengerFactory, startWorkerAndIndexer, bypassSolr);
  ...
  private static ConnectorResult runConnectorInternal(Config config, String runId, Connector connector, Publisher publisher) {
    ...
    log.info(String.format("Connector %s feeding to pipeline %s complete. Time: %.2f secs.", connector.getName(),
        connector.getPipelineName(), durationSecs));
   ...
   */
  private static ConnectorResult runConnectorWithComponents(Config config, String runId, Connector connector,
      WorkerMessengerFactory workerMessengerFactory, IndexerMessengerFactory indexerMessengerFactory,
      PublisherMessengerFactory publisherMessengerFactory, boolean startWorkerAndIndexer, boolean bypassIndexer) throws Exception {
    ...
    try {
      ...
      if (connector.getPipelineName() != null) {
        ...
        publisher = new PublisherImpl(config, publisherMessenger, runId, connector.getPipelineName(), metricsPrefix,
