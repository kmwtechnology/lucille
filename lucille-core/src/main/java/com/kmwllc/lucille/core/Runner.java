 ...
 * Indexer processes that stay alive indefinitely and constantly poll for work. A Runner can then
 * execute a sequence of connectors which pump work into the system. The work from this particular
 * "run" should then be handled by the existing Worker and Indexer. The Runner will terminate when
 * all the work it generated was complete, while the Worker and Indexer would stay alive.
 *
...
public class Runner {
   ...
   * Pipeline workers and indexers will not be run. The assumption is that these have been deployed
