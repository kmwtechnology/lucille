...
public class ConfigValidationTest {
  ...
  public void testApplyRegex() throws DocumentException, JsonProcessingException {
    ...
    testMessage(
        ApplyRegex.class,
        "apply-regex-invalid-type-1.conf",
        doc,
        "source has type BOOLEAN rather than LIST");

    testMessage(
        ApplyRegex.class,
        "apply-regex-invalid-type-2.conf",
        doc,
        "regex has type LIST rather than STRING");

    // multiple variants of the class cast error message have been observed so we only look for a
    // short substring
    testMessage(ApplyRegex.class, "apply-regex-invalid-type-3.conf", doc, "java.lang.String");
  }
  ...
  public void testNonValidationModeException() throws Exception {
    RunResult result =
        Runner.run(
            ConfigFactory.load(addPath("pipelineWithSingleError.conf")), Runner.RunType.TEST);
  ...
  public void testValidationModeException() throws Exception {
    ...
    testException(
        ...
        "com.kmwllc.lucille.stage.NoopStage: "
  ...
  private static void testMessage(
      Class<? extends Stage> stageClass, String config, Document doc, String message) {
    try {
