...
| `otherSQLs` | List\<String\> | No | Additional SELECT queries whose matching rows are attached to each primary Document as nested child Documents. Each query must return rows ordered by its join key. |
