...
export default function ConnectorsPage() {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:8080/v1/config-info/connector-list")
      .then((res) => {
        if (!res.ok) throw new Error("Failed to fetch connector list");
        return res.json();
      })
      .then((data) => {
        setConnectors(data);
  ...
  if (error) {
    return (
      <div className="p-4 sm:p-6 lg:p-8">
        <div className="flex flex-col items-center justify-center min-h-[200px]">
          <p className="text-sm text-red-500">Error loading connectors: {error}</p>
        </div>
      </div>
    );
  }
