 ...
 * with a {@link com.kmwllc.lucille.producer.data.kafkaserde.DocumentSerializer}, and consumed using a
 * {@link org.apache.kafka.clients.consumer.Consumer} set up with a
 * {@link com.kmwllc.lucille.producer.data.kafkaserde.DocumentDeserializer}.
 * <p>
...
public interface DocumentProducer {
  ...
  /**
   * Produces a list of documents from the given file. The {@code doc} param should be set up with basic file
   * information, so if multiple documents are going to be produced, the {@code doc} should be cloned.
   * <p>
   * In the
   * case where no documents should be produced by this file, an empty list should be returned instead of {@code null}.
   * <p>
   * If an error occurs while processing the doc, the method should try to handle it on its own and add a tombstone
   * in the list of documents returned. If a {@link DocumentException} or {@link IOException} occur, the FileTraverser
   * attempts to send a basic tombstone for the document with basic information. Therefore, if this error occurs while
   * processing a child document, throwing an error will prevent the processing of all remaining child documents, and
   * data that may have been usable will be skipped.
   *
   ...
   * will be performed and doc producer will always be run.
   * <p>
   * This check will not be performed if binary data is not being sent, since file size limits and errors will only
   * be applied when sending large file binary data.
   *
  ...
  default String createId(String uniqueKey) {
    try {
      ...
      throw new IllegalStateException("Could not instantiate MD5 message digest for ID creation", e);
    }
  }
  ...
  /**
   * Creates a new Document producer based off of the provided {@code type} after uppercasing it. If an unknown type is
   * provided, a warning is logged and a {@link DefaultDocumentProducer} is returned instead.
   *
  ...
  static DocumentProducer getProducer(String type, boolean childCopyParentMetadata) {
    switch (type.toUpperCase(Locale.ROOT)) {
      case "CSV":
        return new CSVDocumentProducer(childCopyParentMetadata);
      case "DEFAULT":
